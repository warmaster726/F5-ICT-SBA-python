...
class MarkSheetImportPage:
    def __init__(self, master):
        ...
        # Year: Option menu for the recent 5 years, dynamically updated
        tk.Label(self.frame, text="Year:", font=("Arial", 14))\
        ...
        self.year_var.set(self.year_options[0])  # default to current year
        tk.OptionMenu(self.frame, self.year_var, *self.year_options)\
            .grid(row=1, column=1, sticky="w", padx=8, pady=8)

        # Term: Option menu for terms "1" and "2"
        tk.Label(self.frame, text="Term:", font=("Arial", 14))\
            .grid(row=2, column=0, sticky="e", padx=8, pady=8)
        self.term_var = tk.StringVar()
        self.term_var.set("1")  # default value
        term_options = ["1", "2"]
        tk.OptionMenu(self.frame, self.term_var, *term_options)\
            .grid(row=2, column=1, sticky="w", padx=8, pady=8)

        # Exam: Renamed from Exam Name, drop-down containing "Exam"
        tk.Label(self.frame, text="Exam:", font=("Arial", 14))\
            .grid(row=3, column=0, sticky="e", padx=8, pady=8)
        self.exam_var = tk.StringVar()
        self.exam_var.set("Exam")  # default and only option
        exam_options = ["Exam"]
        tk.OptionMenu(self.frame, self.exam_var, *exam_options)\
            .grid(row=3, column=1, sticky="w", padx=8, pady=8)

        # Subject: Standard free text entry
        tk.Label(self.frame, text="Subject:", font=("Arial", 14))\
        ...
        # Papers: Standard free text entry
        tk.Label(self.frame, text="Papers:", font=("Arial", 14))\
            .grid(row=5, column=0, sticky="e", padx=8, pady=8)
        self.papers_var = tk.StringVar()
        tk.Entry(self.frame, textvariable=self.papers_var, font=("Arial", 14))\
            .grid(row=5, column=1, sticky="w", padx=8, pady=8)

        # --- Marksheet File Selection ---
        tk.Label(self.frame, text="Marksheet (Excel):", font=("Arial", 14))\
            .grid(row=6, column=0, sticky="e", padx=8, pady=8)
        ...
        # Adjust grid column weights for proper spacing
        self.frame.grid_columnconfigure(0, weight=1)
        self.frame.grid_columnconfigure(1, weight=3)

    def browse_marksheet_file(self):
        """
        Opens a file dialog to select an Excel file for the marksheet.
    ...
    def on_submit(self):
        # Gather exam parameters from form
        ...
        # Ensure every field is provided
        ...
        # Since the year is selected via an OptionMenu, it will always be valid. Convert it to an integer.
        try:
            year = int(year_str)
        except ValueError:
